...
my $basic_linear_template_missing_edge_ids_v2 = {
		description => q[simple linear chain (stdin->stdout) of nodes],
		version => q[2.0],
		nodes =>
		[
...
my $basic_multipath_template_v2 = {
		description => q[graph with some branching],
		version => q[2.0],
		nodes =>
		[
			{
				...
				use_STDIN => JSON::false,
				use_STDOUT => JSON::true,
				cmd => [ "echo", "Start", "|", "tee", {port => "ALT", direction => "out"} ]
			},
			{
				id => q[nodeA],
				type => q[EXEC],
			...
			},
			{
				...
				use_STDIN => JSON::true,
				use_STDOUT => JSON::true,
				cmd => [ q/tee/, {port => "RIGHT2", direction => "out"} ]
			},
			{
				id => q[tee3],
				type => q[EXEC],
				use_STDIN => JSON::true,
				use_STDOUT => JSON::true,
				cmd => [ q/tee/, {port => "RIGHT3", direction => "out"}, {port => "MID3", direction => "out"} ]
			},
			{
				id => q[Aleft],
				type => q[EXEC],
		...
		[
			{ id => q[e0], from => q[top], to => q[nodeA] },
			{ id => q[e1], from => q[top:ALT], to => q[nodeB] },
			{ id => q[e2], from => q[nodeA], to => q[tee2] },
			{ id => q[e3], from => q[nodeB], to => q[tee3] },
			{ id => q[e4], from => q[tee2], to => q[Aleft] },
			{ id => q[e5], from => q[tee2:RIGHT2], to => q[Aright] },
			{ id => q[e6], from => q[tee3], to => q[Bleft] },
			{ id => q[e7], from => q[tee3:MID3], to => q[Bmid] },
			{ id => q[e8], from => q[tee3:RIGHT3], to => q[Bright] },
		]
};

my $multi_src_template_v2 = {
	version => q[2.0],
	description => q[graph with multiple sources],
	nodes =>
	[
	...
	edges =>
	[
		{ id => q/AP/, from => q/A/, to => q/P:A/, },
		{ id => q/BP/, from => q/B/, to => q/P:B/, },
		{ id => q/CP/, from => q/C/, to => q/P:C/, },
	],
};
...
subtest 'spl00' => sub {
	...
	my $template = $tdir.q[/10-vtfp-splice_nodes_00.json];
	my $template_contents = to_json($basic_linear_template_v2);
	write_file($template, $template_contents);
	...
	is_deeply ($vtfp_results, $expected, '(spl00) one node in a chain spliced out');
	...
	is_deeply ($vtfp_results, $expected, '(spl00) remove all nodes from uc node downstream in the chain (output to STDIN');

	$template = $tdir.q[/10-vtfp-splice_nodes_01.json];
	$template_contents = to_json($basic_linear_template_missing_edge_ids_v2);
	write_file($template, $template_contents);
	...
	is_deeply ($vtfp_results, $expected, '(spl00) one node in a chain (including edges with no ids) spliced out');
	...
	is_deeply ($vtfp_results, $expected, '(spl00) remove all nodes from uc node downstream in a chain including edges with no ids (output to STDIN');
};
...
subtest 'pru00' => sub {
	...
	is_deeply ($vtfp_results, $expected, '(pru00.0) prune final two nodes in a chain (output to STDOUT switched off)');
	...
	is_deeply ($vtfp_results, $expected, '(pru00.0) prune final two nodes in a chain including edge elements with no id (output to STDOUT switched off)');

	$template = $tdir.q[/10-vtfp-prune_nodes_02.json];
	$template_contents = to_json($multi_src_template_v2);
	write_file($template, $template_contents);

	$vtfp_results = from_json(slurp "bin/vtfp.pl -no-absolute_program_paths -verbosity_level 0 -prune_nodes \'-P:A;-P:C\' $template |");
	$expected = {
			version => q[2.0],
			nodes =>
			[
	...
	is_deeply ($vtfp_results, $expected, '(pru00.1) remove two branches specified using implied STDIN src in prune spec');

	$template = $tdir.q[/10-vtfp-prune_nodes_03.json];
	$template_contents = to_json($multi_src_template_v2);
	write_file($template, $template_contents);

	$vtfp_results = from_json(slurp "bin/vtfp.pl -no-absolute_program_paths -verbosity_level 0 -prune_nodes \'-P:(A|C)\' $template |");
	$expected = {
			version => q[2.0],
			nodes =>
			[
	...
	is_deeply ($vtfp_results, $expected, '(pru00.2) remove two branches specified using implied STDIN src in prune spec (pru0.1 with regexp prune spec)');

	$template = $tdir.q[/10-vtfp-prune_nodes_04.json];
	$template_contents = to_json($basic_linear_template_v2);
	write_file($template, $template_contents);
	...
	is_deeply ($vtfp_results, $expected, '(pru00.3) prune final file output node (no range specified; output to STDOUT switched off)');
